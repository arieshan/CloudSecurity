...
public class MessageService implements IMessageServiceLocal, IMessageServiceRemote {
	...
	private String addUser (UserDto dto, OtpAuth otpAuth) throws MessageServiceExn {
		...
		try {
			...
			// TODO add the roles specified in the DTO to the user object (see editUser)
			for(String rname : dto.getRoles()){
				System.out.println(rname);
				roleDAO.getRole(rname).addUser(user);
				//user.addRole(roleDAO.getRole(rname));
			}
	...
	@Override
	// TODO require poster or moderator permission
	@RolesAllowed({"poster","moderator"})
	...
	@Override
	// TODO require poster permission
	@RolesAllowed("poster")
	public long addMessage(MessageDto dto) {
		...
		if (Objects.isNull(principal))  {
			System.out.println("========= >>>> usser name is null in addMessage");
		}
		loggedInUser = principal.getName();
		
		if (!loggedInUser.equals(dto.getSender())) {
			throw new IllegalStateException("Poster of message is inconsistent with message metadata. login user: " + loggedInUser + " sender :" + dto.getSender() + " " + dto.getText() + " " + dto.getTimestamp());
		}
	...
	@Override
	// TODO require moderator permission
	...
	public void checkOtp(String username, Long otpCode) throws MessageServiceExn {
		...
		try {
			 ...
			 * See OneTimePassword.
			 */
			String otpcode = user.getOtpSecret();
			long time = System.currentTimeMillis();
			validOtp = OneTimePassword.checkCode(otpcode, otpCode, time);
