...
public class InitBean {
	...
	private void init() {
		...
		logger.info("Your name here: Xiaohan Change");
		...
		try {
			...
			UserDto cleaner = userDtoFactory.createUserDto();
			cleaner.setUsername("cxh");
			cleaner.setPassword("123456");
			cleaner.setName("Xiaohan Chang");
			cleaner.getRoles().add(POSTER);
			logger.info("Adding user cleaner");
			messageService.addTestUser(cleaner);
			
			UserDto user = userDtoFactory.createUserDto();
			user.setUsername("XiaohanChang");
			user.setPassword("123456");
			user.setName("Xiaohan Chang admin");
			user.getRoles().add(ADMINISTRATOR);
			logger.info("Adding user user");
			messageService.addTestUser(user);
